    ...
    A: TraceVisitorAgent<'w, W>
{
    ...
    {
        match lookup_point {
            LookupPoint::Top => for scope_ref in ctx.waveform.hierarchy().scopes() {
            ...
            LookupPoint::Scope(scope_ref) => {
                ...
                let hier = ctx.waveform.hierarchy();
                let scope = hier.get(scope_ref);
                let mut full_path: Vec<_> =
                    scope.full_name(hier).split('.').map(ToString::to_string).collect();
